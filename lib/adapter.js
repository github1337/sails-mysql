  ...
  var adapter = {
    ...
    stream: function(connectionName, collectionName, options, stream, connection) {
      ...
      function __STREAM__(connection, cb) {
        ...
        var query;
        var data;
        if (stream.customQuery) {
          query = stream.customQuery.query;
          data = stream.customQuery.params;
        } else {
          var sequel = new Sequel(schema, sqlOptions);
        ...
        var dbStream = (function() {
          if (data) return connection.query(query, data);
        ...
        var manualResume = stream.resumeConnectionManually === true;

        // Pausing the connnection is useful if your processing involves I/O
        dbStream.on('result', !manualResume
          ? function dbStreamResultAutoResume(row) {
            connection.pause();
          ...
          : function dbStreamResultManualResume(row) {
            connection.pause();
            stream.write(row, function () {
            });
          }
        ...
        var dbStreamFirstEnd = false;
        // all rows have been received
        dbStream.on('end', function() {
          dbStreamFirstEnd = true;
          stream.end(); // End stream
          cb(); // Close connection
        });

        stream.on('end', function() {
          //console.log('end emitted from stream')
          if(!dbStreamFirstEnd) {
            connection.destroy();
            dbStream.end();
          }
        ...
        stream.resumeConnection = function(){
          connection.resume();
        }

        //stream.on('unpipe', function() { console.log('unpipe emitted from readStream') })
        //stream.on('close', function() { console.log('close emitted from readStream') })
        //stream.on('destroy', function() { console.log('destroy emitted from readStream') })
      }
    },
